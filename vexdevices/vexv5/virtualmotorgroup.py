class MotorGroup(object):
    ...
    def _SetGroupedAttribute(self, attribute: str, value: str or bool or int or float):
        self.Motor1._attributes[attribute] = value
        self.Motor2._attributes[attribute] = value
    ...
    def set_position(self, position, units): self._attributes['position']['value'] = position; self._SetGroupedAttribute('position', position)
    def set_stopping(self, mode): self._attributes['stopping']['value'] = mode; self._SetGroupedAttribute('stopping', mode)
    def set_timeout(self, value, units): pass
    def set_velocity(self, velocity, units): self._attributes['velocity']['value'] = velocity; self._SetGroupedAttribute('velocity', velocity)
    def spin(self, direction): self._attributes['is_spinning']['value'] = True; self._SetGroupedAttribute('is_spinning', True)
    def spin_for(self, direction, angle, units, wait=False): pass
    def spin_to_position(self, angle, units, wait=False): pass
    def stop(self): self._attributes['is_spinning']['value'] = False; self._SetGroupedAttribute('is_spinning', False)
